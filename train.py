...
def main():
    ...
    parser.add_argument(
        "--learning_rate_vae", type=float, default=0.0001, help="learning rate for VAE"
    )
    parser.add_argument(
        "--learning_rate_d", type=float, default=0.001, help="learning rate for D"
    )
    parser.add_argument(
        "--keep_prob", type=float, default=0.65, help="dropout keep probability"
    )
    parser.add_argument(
        "--beta1",
        type=float,
...
def train(args):
    ...
    for epoch in range(training_epochs):
        ...
        avg_q_loss = 0.0
        avg_vae_loss = 0.0
        avg_d_real_accuracy = 0.0
        avg_d_fake_accuracy = 0.0
        avg_g_accuracy = 0.0
        mnist.shuffle_data()
        total_batch = int(n_samples / batch_size)
        # Loop over all batches
        for i in range(total_batch):
            batch_images, batch_labels = mnist.next_batch(
                batch_size, with_label=True
